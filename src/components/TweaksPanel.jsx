...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  return (
    <>
      <button
        style={{position:'fixed',bottom:16,right:16,zIndex:2147483645,
          background:'rgba(41,38,27,.85)',color:'#fff',border:0,borderRadius:10,
...
export function TweakButton({ children, onClick, secondary = false }) {
  return (
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
                    ...
                    onClick={() => onChange(o)}>
              {rest.length > 0 && <span>{rest.map((c, j) => <i key={j} style={{ background: c }} />)}</span>}
            </button>
          );
        })}
      </div>
